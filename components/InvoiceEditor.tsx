...
import { Plus, Trash2, Calendar, Building, Tag, AlertTriangle, FileCheck, FileBox, FileText, History, ShieldAlert, CheckCircle, AlertCircle, Calculator } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-700 slide-in-from-bottom-8">
      ...
      <div className="bg-white/60 dark:bg-slate-800/40 backdrop-blur-sm p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 shadow-sm transition-colors duration-500">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-b border-slate-200 dark:border-white/5 pb-4">
           {/* Document Type Badge */}
            ...
            <div className="flex items-center space-x-2">
               {/* Confidence Badge */}
               <div className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border flex items-center space-x-1.5 ${isHighConfidence ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/30' : 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30'}`}>
               ...
               {data.validationFlags?.lowItemCount && (
                 <div className="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border border-blue-200 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center space-x-1.5" title="Fewer than 3 items found">
                    <AlertCircle className="w-3 h-3" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {/* Vendor Name - Special Inline Edit Style */}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/50">
              {data.lineItems.map((item, index) => {
                ...
                // Base Tab Index for this row = 10 + index * 10
                  ...
                  >
                    ...
                    <td className="p-2 text-center align-top">
                      <button
                        ...
                        tabIndex={-1} // Skip delete in tab flow usually
                      >
